...
## ⚡ One-time setup: download two required libraries

Before loading the extension, you need to add two files:
...
**2. PDF library** (required for quote PDF cross-check) — two files needed:
1. Go to: https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.min.js → save as **`pdf.min.js`**
2. Go to: https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.worker.min.js → save as **`pdf.worker.min.js`**
3. Place both files inside this folder (next to `manifest.json`)
